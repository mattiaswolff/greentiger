<script type="text/javascript">
        $(document).ready(function(){
            ...
            if (getParameterByName("definitionId") != "" ) {
                $.getJSON(getUrlApi("definitions"), { definitionId: strDefinitionId }, function(json) {
                    ...
                    $.each(json.results[0].content, function(key, value) {
                        ...
                        newrow.innerHTML = '<input class="edef-row-title" type="text" name="content[' + counter + '].name" value="' + value.name + '" placeholder="Title" /><select class="edef-row-type" name="content[' + counter + '].type"><option value="text">Text</option><option value="textarea">Textarea</option><option value="email">Email</option><option value="checkbox">Checkbox</option><option value="radio">Radio button</option><option value="date">Date</option><option value="range">Range</option><option value="url">URL</option><option value="number">Number</option><option value="time">Time</option><option value="dropdown">Drop Down</option></select><textarea class="edef-row-config" name="content[' + counter + '].config">' + value.config + '</textarea><textarea class="edef-row-desc" name="content[' + counter + '].description">' + value.type + '</textarea><div class="clear></div>';
            ...
            $(".content").delegate("#addRow", "click", function(){
    	        ...
    	        var strHtml = '<input class="edef-row-title" type="text" name="content[' + counter + '].name" value="' + value.name + '" placeholder="Title" /><select class="edef-row-type" name="content[' + counter + '].type"><option value="text">Text</option><option value="textarea">Textarea</option><option value="email">Email</option><option value="checkbox">Checkbox</option><option value="radio">Radio button</option><option value="date">Date</option><option value="range">Range</option><option value="url">URL</option><option value="number">Number</option><option value="time">Time</option><option value="dropdown">Drop Down</option></select><textarea class="edef-row-config" name="content[' + counter + '].config">' + value.config + '</textarea><textarea class="edef-row-desc" name="content[' + counter + '].description">' + value.type + '</textarea><div class="clear></div>';
