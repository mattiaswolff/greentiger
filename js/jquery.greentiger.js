...
function getTaskFlow (strUserId, strAccessToken, boolEmpty, strSearch) {
    ...
    $.ajax({
        ...
        $.each(json.results[0], function(key, value) {    
            ...
            arrHtml.push('</div></div><div class="left"><a href="' + getUrlClient("dashboard.php?userId=" + value.createdBy._id.$id) +'"><img src="' + getUrlApi("users/" + value.createdBy._id.$id + "?part=image") + '" width="50" height="50" /></a></form><div class="clear" /></article>');
        });
        if (boolEmpty) {
